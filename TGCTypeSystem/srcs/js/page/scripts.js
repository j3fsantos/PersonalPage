...
function loadex () {
	...
	prog_syntax_tree = window.esprima.parse(example.prog_input); 
	prog_str = window.escodegen.generate(prog_syntax_tree, option);
	cm.setValue(prog_str);
...
function typecheck () {
   ...
   try {
      ...
      instrumentation = window.escodegen.generate(output, option);
...
var messageHandler = function(message) {
   if ((message.data === 'IFlow Exception')) {
      alert(message.data);
...
var runCompiledProg = function() {
   ...
   try {
      sendProgramToIFrame(compiled_prog);
   } catch (e) {
...
var runContactManager = function() {
   // 
   str_run_cm = cm_contact_manager_output.getValue();
   mywindow = window.open('cm.html');
   
   setTimeout(function(){ mywindow.postMessage(str_run_cm, window.location); }, 300);
};
